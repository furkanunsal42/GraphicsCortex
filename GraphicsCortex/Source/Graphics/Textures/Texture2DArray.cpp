...
void Texture2DArray::load_data(AsyncBuffer& async_buffer, ColorFormat format, Type type, int mipmap_target)
{
	load_data(async_buffer, format, type, 0, 0, 0, width >> mipmap_target, height >> mipmap_target, texture_count, mipmap_target);
}

void Texture2DArray::load_data(AsyncBuffer& async_buffer, ColorFormat format, Type type, int x, int y, int z, int width, int height, int depth, int mipmap_target)
{
	if (!_texture_generated) {
		std::cout << "[OpenGL Error] released Texture2DArray tried to load_data()" << std::endl;
		ASSERT(false);
	...
	GLCall(glTextureSubImage3D(id, mipmap_target, x, y, z, width, height, depth, TextureBase2::ColorFormat_to_OpenGL(format), TextureBase2::Type_to_OpenGL(type), 0));
...
void Texture2DArray::load_data(AsyncBuffer& async_buffer, DepthStencilFormat format, Type type, int mipmap_target)
{
	load_data(async_buffer, format, type, 0, 0, 0, width >> mipmap_target, height >> mipmap_target, texture_count, mipmap_target);
}

void Texture2DArray::load_data(AsyncBuffer& async_buffer, DepthStencilFormat format, Type type, int x, int y, int z, int width, int height, int depth, int mipmap_target)
{
	if (!_texture_generated) {
		std::cout << "[OpenGL Error] released Texture2DArray tried to load_data()" << std::endl;
		ASSERT(false);
...
void Texture2DArray::copy_to_texture(Texture2DArray& target_texture, int32_t self_mipmap, int32_t target_mipmap)
{
...
void Texture2DArray::copy_to_texture(Texture2DArray& target_texture, int32_t self_mipmap, int32_t target_mipmap, glm::ivec3 copy_size, glm::ivec3 self_offset, glm::ivec3 target_offset)
{
	...
	if (self_offset.x < 0 || self_offset.y < 0 || self_offset.z < 0 || target_offset.x < 0 || target_offset.y < 0 || target_offset.z < 0) {
		std::cout << "[OpenGL Error] Texture2DArray::copy_to_texture() called with negative offset." << std::endl;
		ASSERT(false);
	}

	glm::ivec3 source_mip_size = this->get_size();
	source_mip_size.x = glm::max(1, source_mip_size.x >> self_mipmap);
	source_mip_size.y = glm::max(1, source_mip_size.y >> self_mipmap);

	glm::ivec3 target_mip_size = target_texture.get_size();
	target_mip_size.x = glm::max(1, target_mip_size.x >> target_mipmap);
	target_mip_size.y = glm::max(1, target_mip_size.y >> target_mipmap);
	...
	{
		std::cout << "[OpenGL Error] Texture2DArray::copy_to_texture() copy_size exceeds bounds or is <= 0.\n"
	...
	if (invalid_format)
	{
		std::cout << "[OpenGL Error] Texture2DArray::copy_to_texture() is called with unmatching formated textures" << std::endl;
		ASSERT(false);
	}
...
std::shared_ptr<AsyncBuffer> Texture2DArray::get_image_async(ColorFormat format, Type type, int mipmap_level)
{
	return get_image_async(format, type, mipmap_level, 0, 0, 0, width >> mipmap_level, height >> mipmap_level, texture_count);
}

std::shared_ptr<AsyncBuffer> Texture2DArray::get_image_async(ColorFormat format, Type type, int mipmap_level, int x, int y, int z, int width, int height, int depth)
{
...
std::shared_ptr<AsyncBuffer> Texture2DArray::get_image_async(DepthStencilFormat format, Type type, int mipmap_level)
{
	return get_image_async(format, type, mipmap_level, 0, 0, 0, width >> mipmap_level, height >> mipmap_level, texture_count);
}

std::shared_ptr<AsyncBuffer> Texture2DArray::get_image_async(DepthStencilFormat format, Type type, int mipmap_level, int x, int y, int z, int width, int height, int depth)
{
	...
	if (format == DepthStencilFormat::DEPTH) gl_format = GL_DEPTH_COMPONENT;
		...
		std::cout << "[OpenGL Error] Texture2DArray tried to get_image_async() with unsuppoerted format" << std::endl;
		ASSERT(false);
	}
...
void Texture2DArray::clear(glm::ivec4 clear_data, int mipmap_target)
{
	clear(clear_data, 0, 0, 0, width, height, texture_count, mipmap_target);		
}

void Texture2DArray::clear(glm::uvec4 clear_data, int mipmap_target)
{
	clear(clear_data, 0, 0, 0, width, height, texture_count, mipmap_target);	
}
...
void Texture2DArray::clear(glm::ivec4 clear_data, int x, int y, int z, int width, int height, int depth, int mipmap_target)
{
	if (!_texture_allocated)
		_allocate_texture();
...
void Texture2DArray::clear(glm::uvec4 clear_data, int x, int y, int z, int width, int height, int depth, int mipmap_target)
{
	if (!_texture_allocated)
		_allocate_texture();
