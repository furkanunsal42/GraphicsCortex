...
void Texture2D::_allocate_texture()
{
	...
	if (width >> (mipmap_levels - 1) == 0 || height >> (mipmap_levels - 1) == 0 || mipmap_levels >= sizeof(int) * 8) {
		...
		if (width >> mipmap_levels == 0 || mipmap_levels >= sizeof(int) * 8) mipmap_levels = std::max(1.0, std::log2(width));
		if (height >> mipmap_levels == 0 || mipmap_levels >= sizeof(int) * 8) mipmap_levels = std::max(1.0, std::log2(height));
...
uint32_t Texture2D::get_mipmap_count()
{
...
void Texture2D::clear(glm::ivec4 clear_data, int mipmap_target)
{
	clear(clear_data, 0, 0, width, height, mipmap_target);
}

void Texture2D::clear(glm::uvec4 clear_data, int mipmap_target)
{
	clear(clear_data, 0, 0, width, height, mipmap_target);
}
...
void Texture2D::clear(glm::ivec4 clear_data, int x, int y, int width, int height, int mipmap_target)
{
	if (!_texture_allocated)
		_allocate_texture();
	...
	GLCall(glClearTexSubImage(id, mipmap_target, x, y, 0, width, height, 1, type, GL_INT, &clear_data));
}

void Texture2D::clear(glm::uvec4 clear_data, int x, int y, int width, int height, int mipmap_target)
{
	if (!_texture_allocated)
		_allocate_texture();
	...
	GLCall(glClearTexSubImage(id, mipmap_target, x, y, 0, width, height, 1, type, GL_UNSIGNED_INT, &clear_data));
}
