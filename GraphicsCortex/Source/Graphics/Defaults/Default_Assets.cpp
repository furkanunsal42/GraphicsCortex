...
namespace default_geometry {
	...
	Graphic rectangle(glm::vec2 scale) {
		...
		Graphic g;
		g.set_mesh(Mesh(array_buffer, index_buffer));
		//g.model.array_buffer= array_buffer;
		//g.model.index_buffer = index_buffer;
		return g;
	}

	Graphic rectangle(BindlessMaterial material, glm::vec2 scale) {
		Graphic g = rectangle(scale);
		g.set_material(material);
		return g;
	}

	Graphic rectangle(BindlessMaterial material, glm::ivec2 texture_atlas_dim, const std::vector<unsigned int>& face_texture_locations, glm::vec2 scale) {
		...
		*/
		...
		return Graphic(Mesh(array_buffer, index_buffer), material);
	}
	...
	Graphic cube(BindlessMaterial material, glm::vec3 scale){
		Graphic g = cube(scale);
		g.set_material(material);
		return g;
	}

	Graphic cube(BindlessMaterial material, glm::ivec2 texture_atlas_dim, const std::vector<unsigned int>& face_texture_locations, glm::vec3 scale) {
		float unit_width = 1.0f / texture_atlas_dim.x;
		float unit_height = 1.0f / texture_atlas_dim.y;
		std::vector<glm::vec2> texture_locations;
		for (unsigned int i = 0; i < face_texture_locations.size(); i++) {
