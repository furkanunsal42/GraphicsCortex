...
bool ParsedTexture::exists() const { return channel_count != 0; }
bool ParsedTexture::is_proper() const { return exists() && target_channel != TextureChannel::None; }
...
std::ostream& operator<<(std::ostream& stream, const ParsedMaterial& material)
{
    ...
    for (int32_t i = 0; i < PBR::MAX_TEX_SLOTS; i++) {
...
ParsedMaterial MaterialParser::parse(aiMaterial* mat, const std::filesystem::path& root_path)
 {
...
ParsedTexture MaterialParser::fetch(const std::filesystem::path& root_path, aiMaterial* mat, const std::vector<aiTextureType>& types)
{
    aiString    path;
    bool        found = false;
