...
public:
	...
	void project_to_cubemap					(Texture2D& equirectangular_texture, TextureCubeMap& cubemap_texture);
	void project_to_equirectangular			(Texture2D& equirectangular_texture, TextureCubeMap& cubemap_texture);
...
private:
	...
	VariantedComputeProgram	cp_project_to_cubemap;
	VariantedComputeProgram cp_project_to_equirectangular;
	VariantedComputeProgram cp_compute_irradiance;
	VariantedComputeProgram cp_compute_prefiltered;
	VariantedComputeProgram cp_compute_brdf;
};
