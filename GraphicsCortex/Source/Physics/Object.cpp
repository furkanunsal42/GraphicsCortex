...
void Vehicle::set_mesh_chassis_graphics(std::shared_ptr<Mesh> chassis) {
	this->chassis->set_mesh(chassis);
	chassis_graphic_initialized = true;
}

void Vehicle::set_mesh_left_wheel_graphics(std::shared_ptr<Mesh> left_wheel) {
	this->wheels[FRONT_LEFT]->set_mesh(left_wheel);
	this->wheels[REAR_LEFT]->set_mesh(left_wheel);
	wheel_graphic_initialized = true;
}

void Vehicle::set_mesh_right_wheel_graphics(std::shared_ptr<Mesh> right_wheel) {
	this->wheels[FRONT_RIGHT]->set_mesh(right_wheel);
	this->wheels[REAR_RIGHT]->set_mesh(right_wheel);
	wheel_graphic_initialized = true;
}

void Vehicle::set_mesh_all_graphics(std::shared_ptr<Mesh> chassis, std::shared_ptr<Mesh> left_wheel, std::shared_ptr<Mesh> right_wheel) {
...
void Vehicle::set_material_chassis(std::shared_ptr<BindlessMaterial> chassis) {
	this->chassis->set_material(chassis);
}
void Vehicle::set_material_left_wheel(std::shared_ptr<BindlessMaterial> left_wheel) {
	this->wheels[FRONT_LEFT]->set_material(left_wheel);
	this->wheels[REAR_LEFT]->set_material(left_wheel);
}
void Vehicle::set_material_right_wheel(std::shared_ptr<BindlessMaterial> right_wheel) {
	this->wheels[FRONT_RIGHT]->set_material(right_wheel);
	this->wheels[REAR_RIGHT]->set_material(right_wheel);
}
void Vehicle::set_material_all(std::shared_ptr<BindlessMaterial> chassis, std::shared_ptr<BindlessMaterial> left_wheel, std::shared_ptr<BindlessMaterial> right_wheel) {
	set_material_chassis(chassis);
	set_material_left_wheel(left_wheel);
	set_material_right_wheel(right_wheel);
}
void Vehicle::set_material_all(std::shared_ptr<BindlessMaterial> all) {
