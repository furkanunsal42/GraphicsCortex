...
public:
	...
	void render() {
		auto& dropbox = gui_ref.box(button_aabb, button_style, false);
		...
		dropbox.render();
		for (int i = 0; i < 5; i++) {
			auto& drop_option = gui_ref.box(AABB2(button_aabb.position + vec2(0, button_aabb.size.y) + vec2(0, option_size.y * i), option_size), option_style, false);
			if (panel_show)
				drop_option.render();
		}
