...
int main() {
	...
	Scene scene;
	scene.camera.screen_height = 1920;
	scene.camera.screen_height = 1080;
	scene.camera.max_distance = 1000;

	Program program("Shaders/TextureArray.vert", "Shaders/TextureArray.frag");
	Program solid_program(default_program::solid_program());

	{
		Mesh city("Models/City/edited_city2.obj", 4.0f);
		UnorderedMaterial city_mat("Models/City/edited_city2.obj");
		...
		DirectionalLight sunlight(glm::vec3(1.0f, -0.9f, 0.9f), glm::vec3(0.4, 0.4, 0.4), solid_program);
		sunlight->set_uniform_upadte_queue(default_program::directional_light_default_uniform_queue(sunlight, 0));
		scene.add_light(sunlight);


		AmbiantLight ambinace(glm::vec3(0.1f, 0.1f, 0.1f), solid_program);
		ambinace->set_uniform_upadte_queue(default_program::ambiant_light_default_uniform_queue(ambinace, 0));
